...
public class DisaAktarmaApiTests : IClassFixture<DisaAktarmaApiTests.Factory>
{
    ...
    public async Task Haftalik_csv_json_raporla_ayni_rakamlar()
    {
        ...
        Assert.Equal(["Dönem başı", "Dönem sonu", "Kanal", "Gelen", "Giden", "Sonuç", "Devir"], satirlar[0]);
        ...
        {
            foreach (var k in o.Kanallar.Select(k => (k.Kanal, k.Gelen, k.Giden, k.Sonuc, k.Devir))
                         .Append((CsvRaporlari.KasaSatiri, o.ToplamGelen, o.ToplamGiden, o.KasaSonucu, o.KasaDevir)))
            {
                var s = govde[n++];
    ...
    public async Task Aylik_csv_json_raporla_ayni_rakamlar_ve_toplam(int yil, int ay)
    {
        ...
        Assert.Equal(["Kanal", "Gelen", "Cari giden", "Sabit gider", "Kredi kartı", "Ortak pay", "Ay sonucu"], satirlar[0]);
        var govde = satirlar.Skip(1).SkipLast(1).ToList();
        ...
        for (int i = 0; i < govde.Count; i++)
        {
            ...
            Assert.Equal([k.Gelen, k.CariGiden, k.SabitGider, k.KrediKarti, k.OrtakPay, k.AySonucu], Rakam(govde[i]));
        }
        ...
        Assert.Equal([l.Sum(k => k.Gelen), l.Sum(k => k.CariGiden), l.Sum(k => k.SabitGider), l.Sum(k => k.KrediKarti),
            l.Sum(k => k.OrtakPay), l.Sum(k => k.AySonucu)], Rakam(satirlar[^1]));
    }
